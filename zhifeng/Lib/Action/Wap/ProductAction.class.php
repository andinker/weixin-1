...
class ProductAction extends WapAction {
	...
	public function _initialize() {
		...
		// 分类
		$cats = $this->product_cat_model->where ( array (
				'token' => $this->token 
		) )->order ( 'sortrank asc' )->select ();
		...
		$this->company = M ( 'Company' )->where ( array (
				'token' => $this->token,
				'isbranch' => 0 
		) )->find ();
		// 在线客服
		...
		$wxuser = $this->wxuser;
		if (! $wxuser) {
			$this->wxuser = $wxuser = D ( 'Wxuser' )->where ( array (
					'token' => $this->token 
			) )->find ();
	...
	public function cats() {
		...
		foreach ( $cats as $cat ) {
			...
			$child_ids = $this->getAllChildIds ( $cat['id'] );
				
			$all_ids = strval($cat['id']);
				
			if (! empty ( $child_ids )) {
				$all_cids = implode ( ',', $child_ids );
				$all_ids = $all_ids . ',' . $all_cids;
			}
	...
	public function products() {
		...
		if ($catid) {
			$child_ids = $this->getAllChildIds ( $catid );
			
			$all_ids = strval($catid);
		...
		if (IS_POST) {
			...
			$gourl = U ( 'Product/products', array (
					'token' => $this->token,
					'wecha_id' => $this->wecha_id,
					'keyword' => $key 
			) );
	...
	public function product() {
		...
		if (empty ( $product )) {
			$this->redirect ( U ( 'Product/products', array (
					'token' => $this->token,
					'wecha_id' => $this->wecha_id 
			) ) );
		}
		...
		$normsData = M ( "product_norms" )->where ( array (
				'catid' => $product ['catid'] 
		) )->select ();
		foreach ( $normsData as $row ) {
			$normsList [$row ['id']] = $row ['value'];
		}
		if ($productCatData = M ( 'Product_cat' )->where ( array (
				'id' => $product ['catid'],
				'token' => $this->token 
		) )->find ()) {
		...
		$attributeData = M ( "Product_attribute" )->where ( array (
				'pid' => $product ['id'] 
		) )->select ();
		
		$productDetailData = M ( "Product_detail" )->where ( array (
				'pid' => $product ['id'] 
		) )->select ();
		...
		$productimage = M ( "Product_image" )->where ( array (
				'pid' => $product ['id'] 
		) )->select ();
	...
	public function company($display = 1) {
		...
		// 分店信息
		$branchProducts = $company_model->where ( array (
				'token' => $this->token,
				'isbranch' => 1 
	...
	public function getCat($carts = '') {
		...
		// 免运费处理
		if ($obj = M ( 'Product_setting' )->where ( array (
				'token' => $this->token 
		) )->find ()) {
			if ($obj ['price'] >= 0) {
	...
	public function orderCart() {
		...
		$setting = M ( 'Product_setting' )->where ( array (
				'token' => $this->token 
		) )->find ();
		...
		if (empty ( $totalCount )) {
			$this->error ( '没有购买商品!', U ( 'Product/cart', array (
					'token' => $this->token,
					'wecha_id' => $this->wecha_id 
			) ) );
		}
	...
	public function ordersave() {
		...
		if ($normal_rt) {
			...
			foreach ( $carts as $k => $c ) {
				...
				// 增加销量
				$product_model->where ( array (
						'id' => $k 
				) )->setInc ( 'salecount', $tdata [1] [$k] ['total'] );
			}
		...
		if ($normal_rt) {
			...
			if (empty ( $pay_code )) {
				...
				$this->redirect ( U ( 'Pay/' . $pay_code, array (
						'token' => $token,
						'wecha_id' => $this->wecha_id,
						'orderid' => $orderid 
				) ) );
	...
	public function my() {
		...
		$start = ($page - 1) * $offset;
		$product_cart_model = M ( 'product_cart' );
		$orders = $product_cart_model->where ( array (
				'token' => $this->token,
				'wecha_id' => $this->wecha_id,
				'groupon' => 0 
		) )->limit ( $start, $offset )->order ( 'time DESC' )->select ();
		$count = $product_cart_model->where ( array (
				'token' => $this->token,
				'wecha_id' => $this->wecha_id,
				'groupon' => 0 
		) )->count ();
	...
	public function myDetail() {
		...
		$list = array ();
		if ($cartObj = $product_cart_model->where ( array (
				'token' => $this->token,
				'wecha_id' => $this->wecha_id,
				'id' => $cartid 
		) )->find ()) {
	...
	public function cancelCart() {
		...
		$product_cart_model = M ( 'product_cart' );
		$product_cart_list_model = M ( 'product_cart_list' );
		$thisOrder = $product_cart_model->where ( array (
				'id' => $cartid,
				'token' => $this->token 
		) )->find ();
		if (empty ( $thisOrder )) {
		...
		if (empty ( $thisOrder ['paid'] )) {
			// 删除订单和订单列表
			$product_cart_model->where ( array (
					'id' => $cartid 
			) )->delete ();
			$product_cart_list_model->where ( array (
					'cartid' => $cartid 
			) )->delete ();
			// 还原积分
			$userinfo_model = M ( 'Userinfo' );
			...
			$userinfo_model->where ( array (
					'id' => $thisUser ['id'] 
			) )->setInc ( 'total_score', $thisOrder ['score'] );
			F ( 'fans_token_wechaid', NULL );
			// 商品销量做相应的减少
	...
	public function getPrintcontet($orderid) {
		$thisOrder = M ( 'Product_cart' )->where ( array (
				'orderid' => $orderid 
		) )->find ();
		if ($thisOrder) {
			$orderType = $thisOrder ['groupon'] ? "团购" : "购物";
