...
class YiliaoAction extends BaseAction{
	...
	public function add(){
		if(IS_POST){
			...
			if($this->yiliao_order->add($_POST)){
				...
				$yiliao['title']=$_GET['title'];
				$this->inform($_POST,$yiliao);
	...
	//下单通知
	private function inform($data,$yiliao){
		$token=$data['token'];
		//是否通知开关
		...
		if($inform['yiliao']==1){
			...
			$proInform=unserialize($inform['yiliao']);
		}
		$date=time();
		//邮件,打印条件是否开启
		...
		if($informstatus||$proInform['s']==1){
			...
			if($smsset['phone']){
				$sms_body="您的医疗挂号：《".$yiliao['title']."》于".date('Y-m-d H:i:s',$date)."有新客户参与了，请注意联系客户跟进（".$data['phone']."）";
				$this->Send_sms($smsset['phone'],$sms_body);
			}
		}
		//发邮件
		if($informstatus||$proInform['e']==1){
			$subject="您的医疗挂号《".$yiliao['title']."》又有客户参加了！";
			$body=str_replace("\r\n",'</br>',$info);	
			//邮件列表
