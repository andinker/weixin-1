...
class RenovationAction extends UserAction
{
    public function _initialize()
    {
        parent::_initialize();

        $this->canUseFunction('Renovation');
    }
        ...
        if (IS_POST) {
            if ($es_data == null) {
                if ($id = $data->add($_POST)) {
                    $data1['pid'] = $id;
                    $data1['module'] = 'Renovation';
                    $data1['token'] = session('token');
                    $data1['keyword'] = trim($_POST['keyword']);
                    M('Keyword')->add($data1);
                    $this->success('添加成功', U('Renovation/index', array('token' => session('token'))));
                    die;
                } else {
                ...
                if ($data->where($wh)->save($_POST)) {
                    ...
                    $this->success('修改成功', U('Renovation/index', array('token' => session('token'))));
                } else {
                    $this->error('操作失败');
                }
            }
    ...
    public function son_add()
    {
        ...
        if (IS_POST) {
            if ($check == null) {
                ...
                if ($t_son->add($_POST)) {
                    $this->success('添加成功', U('Renovation/son', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_son->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Renovation/son', array('token' => session('token'))));
                } else {
                    $this->error('操作失败');
                }
            }
    ...
    public function son_del()
    {
        ...
        $id = (int) $this->_get('id');
        $token = $this->_get('token');
        $where = array('id' => $id, 'token' => $token,'type' => 'Renovation');
        $check = $t_son->where($where)->find();
        if ($check == null) {
            $this->error('操作失败');
        } else {
            ...
            if ($isok) {
                $this->success('删除成功', U('Renovation/son', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Renovation/son', array('token' => session('token'))));
            }
        }
    }
    ...
    public function housetype_add()
    {
        ...
        $son_data = M('Estate_son')->where(array('token' => session('token'),'type' => 'Renovation'))->field('id as sid,title')->select();
        $this->assign('son_data', $son_data);
        ...
        if (IS_POST) {
            if ($check == null) {
                ...
                if ($t_housetype->add($_POST)) {
                    $this->success('添加成功', U('Renovation/housetype', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_housetype->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Renovation/housetype', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('操作失败');
                }
    ...
    public function housetype_del()
    {
        ...
        if ($check == null) {
            ...
            if ($isok) {
                $this->success('删除成功', U('Renovation/housetype', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Renovation/housetype', array('token' => session('token'))));
            }
        }
    }
    ...
    public function album_add()
    {
        ...
        if (IS_POST) {
            if ($check == NULL) {
                ...
                $_POST['token'] = session('token');
				$_POST['type']= 'Renovation';
                if ($t_album->add($_POST)) {
                    $this->success('添加成功', U('Renovation/album', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_album->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Renovation/album', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('操作失败');
                }
    ...
    public function album_del()
    {
        ...
        if ($check == null) {
            ...
            $isok = $housetype->where($where)->delete();
            if ($isok) {
                $this->success('删除成功', U('Renovation/album', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Renovation/album', array('token' => session('token'))));
            }
        }
    }
        ...
        if (IS_POST) {
            if ($check == null) {
                if ($t_impress->add($_POST)) {
                    $this->success('添加成功', U('Renovation/impress', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_impress->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Renovation/impress', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('操作失败');
                }
    ...
    public function impress_del()
    {
        ...
        if ($check == null) {
            ...
            if ($isok) {
                $this->success('删除成功', U('Renovation/impress', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Renovation/impress', array('token' => session('token'))));
            }
        }
    }
        ...
        if (IS_POST) {
            ...
            if ($check == null) {
                if ($t_expert->add($_POST)) {
                    $this->success('添加成功', U('Renovation/expert', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('服务器繁忙,请稍候再试');
                }
                ...
                if ($t_expert->where($wh)->save($_POST)) {
                    $this->success('修改成功', U('Renovation/expert', array('token' => session('token'))));
                    die;
                } else {
                    $this->error('操作失败');
                }
    ...
    public function expert_del()
    {
        ...
        if ($check == null) {
            ...
            if ($isok) {
                $this->success('删除成功', U('Renovation/expert', array('token' => session('token'))));
            } else {
                $this->error('删除失败', U('Renovation/expert', array('token' => session('token'))));
            }
        }
    }
    public function reservation()
    {
        ...
        $t_company = M('Company');
        $token = session('token');
        $where =  array('token'=>$token,'isbranch'=>1,'shortname'=>'Renovation');
        ...
        if(IS_POST){
            ...
            if($check == null){     
                    ...
                    if($t_company->add($_POST)){                     
                        $this->success('添加成功',U('Renovation/aboutus',array('token'=>session('token'))));
                        exit;
                    }else{
             ...
             if($t_company->where($wh)->save($_POST)){ 
                    $this->success('修改成功',U('Renovation/aboutus',array('token'=>session('token'))));
                    exit;
                }else{
