...
class Wechat_groupAction extends UserAction{
    ...
    public function _initialize(){
        parent :: _initialize();
        $this->canUseFunction('fensi');
        $where = array('token' => $this -> token);
		$api=M('wxuser')->field('appid,appsecret,id')->where(array('token'=>session('token')))->find();
        ...
        if (!$this -> thisWxUser['appid'] || !$this -> thisWxUser['appsecret']){
            $this -> error("请先设置AppID和AppSecret再使用本功能，谢谢", '?g=User&m=Index&a=edit&token='.$this -> token.'&id='. session('wxid'));
        }
    }
