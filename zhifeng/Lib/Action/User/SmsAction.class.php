...
class SmsAction extends UserAction{
	...
	public function sends(){
		...
		if(!$set['status']){
			$this->error('请先开启功能！',U('Sms/index',array('token'=>$this->token)));
		}
		$txt = $this->Send_sms($set['phone'],"您好，这是测试短信接口功能呢");
		if($txt){
			$this->success('短信发送成功',U('Sms/index',array('token'=>$this->token)));
		}else{
			$this->error('短信发送失败',U('Sms/index',array('token'=>$this->token)));
		}
	}
