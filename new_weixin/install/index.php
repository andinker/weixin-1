...
'license'=>'<div class="license"><h1>系统安装协议</h1>
			...
			<strong>三、协议规定的约束和限制 </strong>
				...
				<p>3、不管你的网站是否整体使用智风微信营销平台 ，还是部份栏目使用智风微信营销平台，在你使用了智风微信营销平台 的网站主页上必须加上智风微信营销平台   官方网址(<a href="http://weixin.5cando.com" target="_blank">智风微信</a>)的链接。</p>
				<p>4、未经官方许可，禁止在智风微信营销平台的整体或任何部分基础上以发展任何派生版本、修改版本或第三方版本用于重新分发。</p>
			...
			<strong>四、有限担保和免责声明 </strong>
				...
				<p>3、电子文本形式的授权协议如同双方书面签署的协议一样，具有完全的和等同的法律效力。您一旦开始确认本协议并安装智风微信营销平台，即被视为完全理解并接受本协议的各项条款，在享有上述条款授予的权力的同时，受到相关的约束和限制。协议许可范围以外的行为，将直接违反本授权协议并构成侵权，我们有权随时终止授权，责令停止损害，并保留追究相关责任的权力。</p>
				<p>4、如果本软件带有其它软件的整合API示范例子包，这些文件版权不属于本软件官方，并且这些文件是没经过授权发布的，请参考相关软件的使用许可合法的使用。</p>
				<p><b>版本最新更新：</b> 2013年12月28日</p>
                <p align=right>BY:智风微信营销平台</p>
</div>',
'agreement_yes' => '我同意，继续安装',
'agreement_no' => '我不同意',
'title_install' => ' 智风微信营销平台 V'.SOFT_VERSION.' 安装程序',
...
if($method == 'show_license') {//声明
	...
	if($submit) {
		...
		if($confstatus['status']==1){
			...
			$sql = str_replace("\r\n", "\n", $sql);
			$sql = str_replace("zfwx_",$tablepre, $sql);//
			show_install();
			runquery($sql);

			$sqlfile = 'sql/initdata.sql';
			$sql = file_get_contents($sqlfile);
			$sql = str_replace("\r\n", "\n", $sql);
			runquery($sql);
...
<meta content="智风微信营销平台" name="Copyright" />
...
?>
	...
	<div class="main">
		...
		<div class="btnbox marginbot" id="message1" style="display: none;">
...
function runquery($sql) {
	...
	$sql = str_replace("\r", "\n", str_replace('zfwx_', $tablepre, $sql));
	$ret = array();
	$num = 0;
	foreach(explode(";\n", trim($sql)) as $query) {
		$ret[$num] = '';
