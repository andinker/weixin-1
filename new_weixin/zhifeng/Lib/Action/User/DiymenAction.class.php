...
class DiymenAction extends UserAction{
	...
	public function  class_send(){
		if(IS_GET){
			$api=M('wxuser')->field('appid,appsecret')->where(array('token'=>session('token')))->find();
			...
			if($json->access_token){
				...
				$statusss=$this->api_notice_increment($url,$data);
				if($statusss==false){
					$this->error('操作失败');
				}else{
					$statusss=json_decode($statusss);
					if($statusss->errcode){
						$str='生成菜单失败,错误代码:'.$statusss->errcode.'错误信息:'.$statusss->errmsg;
						$this->error($str);
					}else{					
				...
				$this->error('生成access_token出错，错误代码：'.$json->errcode);
			}
			exit;
		}else{
			$this->error('非法操作');
