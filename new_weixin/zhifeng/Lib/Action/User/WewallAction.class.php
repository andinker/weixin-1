...
class WewallAction extends UserAction{
	...
	{
		$uid=$this->_post('uid');
		$where=array('token'=>$this->_post('token'),'uid'=>$uid,'ifsent'=>'0','ifcheck'=>'1');
		$result=M('Wewalllog')->field('content')->where($where)->order('updatetime desc')->select();
