...
class AlipayAction extends UserAction{
	...
	public function post(){
		...
		if($price==false||$this->_post('uname')==false)$this->error('价格和用户名必须填写');
		if($gid!=$this->_session('gid')&&$this->_session('viptime')>time())$this->error('如需升级请联系客服！');
		...
		$data=M('Indent')->data(array('uid'=>session('uid'),'title'=>$subject,'uname'=>$this->_post('uname'),'gid'=>$gid,'create_time'=>time(),'indent_id'=>$out_trade_no,'price'=>$total_fee,'quantity'=>$this->_post('num')))->add();
	...
	public function return_url (){
		...
		if($verify_result) {
			...
			$reurl=U('Home/Index/index');
			if($trade_status == 'TRADE_FINISHED' || $trade_status == 'TRADE_SUCCESS') {
					$indent=M('Indent')->field('id,uid,price,quantity,gid,status')->where(array('index_id'=>$out_trade_no))->find();
					if($indent!==false){
						//$back=M('Users')->where(array('id'=>$indent['uid']))->setInc('money',$indent['price']);
						if($order['status']==0){
							//自动完成注册时间增加和等级变更
							$data['status'] = 2;
							$viptime=session('viptime')+$indent['uid']*30*24*3600;
							session('viptime',$viptime);
							session('gid',$indent['gid']);
							$usersdb=M('Users');
							$users=$this->user;
							$usersdb->where(array('id'=>$indent['uid']))->save(array('viptime'=>$users['viptime']+$viptime,'gid'=>$indent['gid']));
							/***/
							$back=M('Indent')->where(array('index_id'=>$out_trade_no))->save($data);
							if($back!=false){
								$this->success('充值成功',$reurl);
							}else{
								$this->error('充值失败,请联系在线客服,为您处理',$reurl);
							}
						}
					}else{						
						$this->error('订单不存在',$reurl);
					
					}
			}else {			
				$this->error('请勿重复操作',$reurl);
			}
		}else {			
			$this->error('充值失败 ,请联系在线客服,为您处理',$reurl);
	...
	public function notify(){
		...
		if($verify_result) {//验证成功
			...
			$order =  M('Indent')->where(array('index_id'=>$out_trade_no))->find();
			if($trade_status == 'TRADE_FINISHED' || $trade_status == 'TRADE_SUCCESS') {
				if($order['status']==0){
					//自动完成注册时间增加和等级变更
					$data['status'] = 2;
					...
					$usersdb->where(array('id'=>$indent['uid']))->save(array('viptime'=>$users['viptime']+$viptime,'gid'=>$indent['gid']));
					/***/
					M('Indent')->where(array('index_id'=>$out_trade_no))->save($data);
				}
				echo "success";	
			}	
		}
