...
class CardAction extends BaseAction
{
    ...
    function card()
    {
		...
		$membercardsetWhere['token']=$token;
		if($cardid){
			$membercardsetWhere['id']=$cardid;
		}
        $thisCard           = $member_card_set_db->where($membercardsetWhere)->find();
        $this->assign('thisCard', $thisCard);
        ...
        if ($thisCard) {
            ...
            $this->assign('userScore', $userScore);
            $member_card_create_db = M('Member_card_create');
			$mccWhere['token']=$token;
			$mccWhere['wecha_id']=$wecha_id;			
			$mccWhere['cardid']=$cardid;
			
            $thisMember            = $member_card_create_db->where($mccWhere)->find();
            $hasApplied            = count($thisMember);
            if (!$hasApplied) {
                $card = M('Member_card_create')->field('id,number')->where("token='" . $token . "' and cardid=" . $thisCard['id'] . " and wecha_id = ''")->find();
                if (!$card) {
                    $error = -1;
                } else {
                    if (intval($thisCard['miniscore']) == 0 || $userScore > intval($thisCard['miniscore'])) {
                        $error = -4;
                        header('Location:'.U('Userinfo/index',array('token'=>$token,'wecha_id'=>$wecha_id,'cardid'=>$thisCard['id']))); 
                    } else {
                        $error = -3;
                    }
                }
