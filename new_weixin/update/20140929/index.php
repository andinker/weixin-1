...
$update_banben='20140930';
$need_banben='20140922';
$ver='2.3';
...
if(!pdo_fieldexists("{$dbpre}dining_company",'sctime')){
	...
	if($sql){
		echo '表 dining_company 字段添加成功';
	}else{
		echo '表 dining_company 字段增加失败';
	}
	echo "</br> ";
}

if(!pdo_fieldexists("{$dbpre}home",'bjdh')){
	$sql=mysql_query("ALTER TABLE `{$dbpre}home` ADD `bjdh` tinyint(2) NOT NULL COMMENT '背景动画', ADD `yxslide` tinyint(2) NOT NULL COMMENT '动画优先显示';");
	if($sql){
		echo '背景动画字段添加成功';
	}else{
		echo '背景动画字段增加失败';
